...
<?foreach ($arResult["SITES"] as $key => $arSite):?>
	...
	<?else:?>
		<li><a class="lang-box__item" href="<?if(is_array($arSite['DOMAINS']) && strlen($arSite['DOMAINS'][0]) > 0 || strlen($arSite['DOMAINS']) > 0):?><?=$isHttps?>://<?endif?><?=(is_array($arSite["DOMAINS"]) ? $arSite["DOMAINS"][0] : $arSite["DOMAINS"])?><?=$arSite["DIR"]?>" title="<?=$arSite["LANG"]?>"><?=$arSite["LANG"]?></a></li>
	<?endif?>
<?endforeach;?>
</ul>